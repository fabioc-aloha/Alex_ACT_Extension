...
function installEditionBrain(workspaceRoot) {
    ...
    // Transactional install: copy to a temp dir first, then swap via rename.
    // This prevents a half-written .github/ if the process is killed mid-copy.
    const tempDir = ghDir + '-act-install-' + Date.now();
    try {
        fs.cpSync(BRAIN_DIR, tempDir, { recursive: true });
        // Rename old .github/ out of the way (if it exists)
        const oldDir = ghDir + '-act-old-' + Date.now();
        if (fs.existsSync(ghDir)) {
            fs.renameSync(ghDir, oldDir);
        }
        // Promote new brain
        fs.renameSync(tempDir, ghDir);
