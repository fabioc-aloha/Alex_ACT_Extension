 ...
 * @param {{ authToken?: string | null, timeoutMs?: number, httpGetJson?: typeof _httpGetJson, resolveCommitSha?: typeof resolveCommitSha }} [opts]
...
async function getLatestTag(extensionVersion, globalState, opts) {
    ...
    const httpGetJson = (opts && opts.httpGetJson) || _httpGetJson;
    const resolveSha = (opts && opts.resolveCommitSha) || resolveCommitSha;
    ...
    const res = await httpGetJson(url, headers, timeoutMs);
    ...
    if (res.statusCode === 200 && res.body) {
        ...
        let commitSha = await resolveSha(json.target_commitish, tag, extensionVersion, authToken, timeoutMs);
        const payload = {
            tag,
            commitSha,
...
async function fetchTarball(tag, extensionVersion, opts) {
    ...
    const extract = (opts && opts.extract) || fetchAndExtract;
    ...
    try {
        await extract(url, tempParent, { headers, timeoutMs });
    } catch (err) {
