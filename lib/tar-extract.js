        ...
        gunzip.on('data', (chunk) => {
            ...
            try {
                while (buffer.length >= BLOCK) {
                    ...
                    // End-of-archive marker. The strict POSIX spec calls for
                    // two consecutive zero blocks; in practice every tar.gz
                    // GitHub serves is well-formed and the first zero header
                    // (name byte 0) reliably signals end of meaningful data.
                    // We accept the single-block signal and drain the rest.
                    // If a future GitHub-served tarball ever emits leading
                    // zero bytes inside a real header, switch to two-block
                    // validation before treating as EOF.
                    if (header[0] === 0) {
